...
public class PassphraseRequiredMixinSSS {
  ...
  public void onStop(Context context, PassphraseRequiredActivitySSS activity) {
	  removeServiceConnection(context);// not sure if this is necessary, tryign to combat the leaked ServiceConnection exception
  }

  public void onDestroy(Context context, PassphraseRequiredActivitySSS activity) {
	  removeServiceConnection(context);// not sure if this is necessary, tryign to combat the leaked ServiceConnection exception
	  
	  removeClearKeyReceiver(context);
  }

  private void initializeClearKeyReceiver(Context context, final PassphraseRequiredActivitySSS activity) {
    ...
    IntentFilter filter = new IntentFilter(KeyCachingServiceSSS.CLEAR_KEY_EVENT);
    context.registerReceiver(clearKeyReceiver, filter, KeyCachingServiceSSS.KEY_PERMISSION, null);
  }

  private void initializeNewKeyReceiver(Context context, final PassphraseRequiredActivitySSS activity) {
    ...
    IntentFilter filter = new IntentFilter(KeyCachingServiceSSS.NEW_KEY_EVENT);
    context.registerReceiver(newKeyReceiver, filter, KeyCachingServiceSSS.KEY_PERMISSION, null);
  }

  private void initializeServiceConnection(Context context, PassphraseRequiredActivitySSS activity) {
  ...
  private static class KeyCachingServiceConnection implements ServiceConnection {
    private final PassphraseRequiredActivitySSS activity;

    private boolean isBound;

    public KeyCachingServiceConnection(PassphraseRequiredActivitySSS activity) {
    ...
    public void onServiceConnected(ComponentName name, IBinder service) {
      KeyCachingServiceSSS keyCachingService  = ((KeyCachingServiceSSS.KeyCachingBinder)service).getService();
